...
const BannerForm = ({ onSubmit, loading, existingBanner, isEdit = false }: Props) => {
  ...
  return (
      <Stack spacing={2} component="form" onSubmit={submitHandler} sx={{ maxWidth: 650, mx: "auto", mt: 4 }}>
        ...
        <Box>
          <FileInput label={("Изображение ")} name="image" onChange={imageHandler}/>
            {state.image && (
          ...
          <Button
              ...
              sx={{ bgcolor: "#660033" }}
          >
              {loading ? (isEdit ? "Обновление..." : "Создание...") : (isEdit ? "Обновить" : "Создать")}
        </Button>
      </Stack>
  );
};
