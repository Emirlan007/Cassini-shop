...
import { fetchFilteredProducts, fetchProducts } from "../features/products/productsThunks";
import {
    selectProducts,
...
const CategoryProductsPage = () => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        if (availableOptions.priceRange.min > 0 && availableOptions.priceRange.max > 0) {
            setFilters(prev => ({
                ...prev,
                priceRange: [availableOptions.priceRange.min, availableOptions.priceRange.max]
            }));
    ...
    useEffect(() => {
        if (categoryId && availableOptions.priceRange.min > 0) {
            const filterParams: FilterParams = {
                ...
                minPrice: filters.priceRange[0] !== availableOptions.priceRange.min ? filters.priceRange[0] : undefined,
                maxPrice: filters.priceRange[1] !== availableOptions.priceRange.max ? filters.priceRange[1] : undefined,
                inStock: filters.inStock !== undefined ? filters.inStock : undefined,
                isNew: filters.isNew !== undefined ? filters.isNew : undefined,
                isPopular: filters.isPopular !== undefined ? filters.isPopular : undefined,
        ...
        <Container maxWidth="xl" sx={{ py: { xs: 3, sm: 4 } }}>
            ...
            {hasNoProducts ? (
                ...
                >
                    ...
                    >
                        Попробуйте изменить параметры фильтров или выбрать другую категорию
                    </Typography>
                    <Button
                        variant="contained"
